...
automatically bump package versions in values.yaml files using the updatecli CLI tool.
"""
...
# Add repositories here where it is known that higher versions are not
...
def update_values(path_chart: str):
   ...
   for i,dependency in enumerate(chart["dependencies"]):
       ...
       if dependency["name"] in EXCLUDED_REPOS:
...
if __name__ == "__main__":
   ...
   for path in paths.iterdir():
       try:
           update_values(str(path.absolute()))
       except Exception as e:
           print(f"Failed processing values {path}")
